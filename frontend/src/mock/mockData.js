...
//
// 2) USERS FOR USER MANAGEMENT (unchanged)
//
export const users = [
    {
        id: 1,
...
//
// 4) HELPER DAYS + getFixedDateForDay
//
const days = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];

export function getFixedDateForDay(day) {
    const mapping = {
        Sunday:    '2025-01-05',
        Monday:    '2025-01-06',
        Tuesday:   '2025-01-07',
...
//
// 5) SINGLE-WEEK SCHEDULE (LEGACY)
//   24-hour coverage approach: 20 employees * 5 days => 100 shifts
//
let assignments = [];
...
//
// 6) MULTI-WEEK SCHEDULE: "2025-W-1", "2025-W00", "2025-W01", "2025-W02", "2025-W03"
//
const weekCodes = ["2025-W-1", "2025-W00", "2025-W01", "2025-W02", "2025-W03"];

/**
 * Generate a random coverage for each of the 5 weeks, same 24-hour logic,
 * so every day is filled with 14/15 shifts (non-empty).
 */
function generateWeekCoverage() {
    // create 100 random assignments from allEmployees
    let assignmentsArray = [];
    allEmployees.forEach(emp => {
        for (let i = 0; i < emp.maxDays; i++) {
            assignmentsArray.push(emp.id);
        }
    });
    // shuffle
    for (let i = assignmentsArray.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [assignmentsArray[i], assignmentsArray[j]] = [assignmentsArray[j], assignmentsArray[i]];
    }
    ...
    days.forEach(day => {
        ...
        const spacing = (24 - 8) / (numShifts - 1);
        for (let i = 0; i < numShifts; i++) {
            const empId = assignmentsArray[idx++];
            const emp = allEmployees.find(e => e.id === empId);
            const startHour = Math.floor(i * spacing);
            const endHour = (startHour + emp.maxHours) % 24;
...
// Build dictionary of 5 weeks
export const fullWeekScheduleByWeek = {};
weekCodes.forEach(code => {
    fullWeekScheduleByWeek[code] = generateWeekCoverage();
});

//
// 7) OPTIONAL dummySchedules + dummyNotes
//
